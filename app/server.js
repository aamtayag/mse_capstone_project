...
app.use(session({
  store: new SQLiteStore({ db: 'sessions.sqlite' }),  // Use persistent storage!
...
app.get('/admin/load-hazards', (req, res) => {
   ...
   else if (sort === 'creation') orderBy = 'r.created_at DESC';
...
app.get('/api/analytics', async (req, res) => {
   ...
   try {
      ...
      const { total } = await dbGetAsync('SELECT COUNT(*) AS total FROM road_hazards where hazard_status_id not in (5,6)');
